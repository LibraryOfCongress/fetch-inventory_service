...
class Aisle(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("building_id", "aisle_number_id", name="uq_building_aisle_number_id"),
        UniqueConstraint("module_id", "aisle_number_id", name="uq_module_aisle_number_id"),
        sa.CheckConstraint("(module_id IS NULL AND building_id IS NOT NULL) OR (building_id IS NULL AND module_id IS NOT NULL)", name="ck_building_xor_module"),
    )

    id: Optional[int] = Field(primary_key=True, sa_column=sa.Integer, default=None)
    aisle_number_id: int = Field(
        foreign_key="aisle_numbers.id",
        nullable=False,
        default=None
    )
    module_id: Optional[int] = Field(foreign_key="modules.id", nullable=True, default=None)
    building_id: Optional[int] = Field(foreign_key="buildings.id", nullable=True, default=None)
