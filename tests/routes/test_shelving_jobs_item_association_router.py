...
def test_get_all_shelving_jobs_item_association(client):
    response = client.get("/shelving-jobs/items-association")
...
def test_get_shelving_jobs_item_association_by_page(client):
    response = client.get("/shelving-jobs/items-association?page=1")
    assert response.status_code == status.HTTP_200_OK
...
def test_get_shelving_jobs_item_association_by_page_size(client):
    response = client.get("/shelving-jobs/items-association?size=10")
    assert response.status_code == status.HTTP_200_OK
...
def test_get_all_shelving_jobs_item_association_not_found(client):
    response = client.get("/shelving-jobs/items-association/999")
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "Not Found"}


def test_get_shelving_job_item_association_by_id(client):
    response = client.get("/shelving-jobs/items-association/1")
    assert response.status_code == status.HTTP_200_OK
    assert response.json().get(
        "id"
    ) == SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD_RESPONSE.get("items")[0].get("id")


def test_create_shelving_job_item_association_record(client):
    response = client.post(
        "/shelving-jobs/items-association",
        json=CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD,
    )
    assert response.status_code == status.HTTP_201_CREATED
    assert response.json().get(
        "verified"
    ) == CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get("verified")
    assert response.json().get(
        "shelf_position_proposed_id"
    ) == CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get(
        "shelf_position_proposed_id"
    )
    assert response.json().get(
        "shelf_position_id"
    ) == CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get("shelf_position_id")
    assert response.json().get(
        "shelving_job_id"
    ) == CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get("shelving_job_id")
    assert response.json().get(
        "item_id"
    ) == CREATE_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get("item_id")


def test_patch_shelving_job_item_association_record(client):
    response = client.patch(
        "/shelving-jobs/items-association/1",
    ...
    assert response.json().get(
        "verified"
    ) == UPDATED_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD.get("verified")


def test_update_shelving_job_item_association_record_not_found(client):
    response = client.patch(
        "/shelving-jobs/items-association/999",
        json=UPDATED_SHELVING_JOBS_ITEM_ASSOCIATION_SINGLE_RECORD,
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_delete_shelving_job_item_association_record_not_found(client):
    response = client.delete("/shelving-jobs/items-association/999")
