...
down_revision: Union[str, None] = '2024_02_06_12:41:18'
...
def upgrade() -> None:
    ...
    op.add_column('non_tray_items', sa.Column('size_class_id', sa.Integer(), nullable=True))
    op.drop_constraint('non_tray_items_tray_size_class_id_fkey', 'non_tray_items', type_='foreignkey')
    op.create_foreign_key(None, 'non_tray_items', 'size_class', ['size_class_id'], ['id'])
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('trays', sa.Column('tray_size_class_id', sa.INTEGER(), autoincrement=False, nullable=False))
    ...
    op.add_column('shelves', sa.Column('tray_size_class_id', sa.INTEGER(), autoincrement=False, nullable=False))
    ...
    op.add_column('non_tray_items', sa.Column('tray_size_class_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'non_tray_items', type_='foreignkey')
    op.create_foreign_key('non_tray_items_tray_size_class_id_fkey', 'non_tray_items', 'tray_size_class', ['tray_size_class_id'], ['id'])
    op.drop_column('non_tray_items', 'size_class_id')
    op.add_column('items', sa.Column('tray_size_class_id', sa.INTEGER(), autoincrement=False, nullable=True))
    ...
    sa.Column('id', sa.SMALLINT(), server_default=sa.text("nextval('tray_size_class_id_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.Column('name', sa.VARCHAR(length=25), autoincrement=False, nullable=False),
    sa.Column('update_dt', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
    sa.Column('create_dt', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
